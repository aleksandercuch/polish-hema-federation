...
const ArticleFormControl = (props: IProps) => {
        ...
        async (
            ...
            if (newMainFile instanceof File) {
                const mainFileRef = ref(
                    storage,
                    `articles/${addRandomSuffix(newMainFile.name)}`
                );
    ...
    const handleImageUploads = useCallback(async (): Promise<string[]> => {
        ...
        const uploadPromises = newFiles.map(async (file) => {
            const storageRef = ref(
                storage,
                `articles/${addRandomSuffix(file.name)}`
            );
    ...
    const submitForm = useCallback(
        async (data: PostT) => {
            ...
            try {
                const postRef = post.id ? doc(db, "articles", post.id) : null;
